...
1. If the command implements `ConfirmableCommand` (such as `edit`, `delete`, `clear`, or `exit`), `LogicManager` does not execute it immediately. Instead, it temporarily saves the command as a pending confirmation and prompts the user for a `y`/`n` input.
1. Upon confirming (`y`), the command can communicate with the `Model` when it is executed (e.g. to delete an employee).<br>
   Note that although this is shown as a single step in the diagram above (for simplicity), in the code it can take several interactions (between the command object and the `Model`) to achieve.
1. The result of the command execution or confirmation prompt is encapsulated as a `CommandResult` object which is returned back from `Logic`.
